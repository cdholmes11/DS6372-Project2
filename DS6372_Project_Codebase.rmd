...
```
## weight,bmi  are corelated to eachother
## fracscore and age are highly co-related to eachother. So it is decided to use only weight,height and age for our LDA and QDA model. 
...
##output 75.12 = No
  ##  24.87 % = Yes that means in our dataset there are only 25 % of subjects have fracture. The coefficents of LDA provides the linear combination of variables that are used to form LDA decision rule. The output group means indicates the mean values of different feature when they fall to a particular response class.THe group means difference between the age,weight and height for the fracture  repsonse is not significant so it wont be easier to classify.   

plot(lda.m,col=lq_train$fracture)

lda.m.pred.train <- predict(lda.m,lq_train)
lda.m.pred.train
ldahist(lda.m.pred.train$x[,1],g=lda.m.pred.train$class)

## The  plot shows how the response class has been classified by the LDA classifier. The X-axis shows the value of line defined by the co-efficient of linear discriminant for LDA model. The two groups are the groups for response classes. from the histogram we can see that when it is greater than 2 , the probability of fracture increases.

lda.m.pred.t <- predict(lda.m,lq_test)
lda.m.pred.t
ldahist(lda.m.pred.t$x[,1],g=lda.m.pred.t$class)

## Same for test set. from the histogram we can see that when it is greater than 2 , the probability of fracture increases.
...
##########ERROR METRICS##############################################
## from ROC curve the threshold is 0.299~0.3 and AUC of 0.735. TPR(sensitivity),FPR(specificity) the value closer to (0,1) is determined as probabilty threshold . In this case the threshold is 0.299
...
## the accuracy of our LDA model on test data is 77.08 % and error rate is 22.92%.## sensitivity aka accuracy of true positive class (who has fracture) is only 57.14% And specificity aka accuracy of true negative class (who dont have fracture) is 85.29%. 
##PPV - is 61.54% is predicted positive (YES) among all the positive (Yes) predictions
##NPV - 82.86 % is predicted negative (No) among all the negative (NO) predictions

### SO the ppv value is needed in order to find new patient going to have fracture and need medication is only 61.54%. Since our NPV is 82.86 % that means we can assure that the patient is not going to have fracture 82.86 accuracy.

###Prevalence is the chances of being the + class with no information,that means on only 29.17% yes fracture class shows up. 
...
######### This figure shows how test data has been classified using QDA model. The mix of red and black colored shows incorrect classification prediction. 
...
##from ROC curve the threshold is 0.285~0.29 and AUC of 0.739. TPR(sensitivity),FPR(specificity) the value closer to (0,1) is determined as probabilty threshold . In this case the threshold is 0.29
...
## Sensitivity ##64.29 %
##Specificity 82.35 %
##PPV is only 60 % little worse than LDA , so this model cannot be used if we want to predict if someone going to have fracture.
##NPV is 84.85 
## Prevalence is 29.17%
...
legend("bottomright",
       legend=c("Logistic -AUC = 0.765", "LDA -AUC = 0.735","QDA -AUC =0.739 "),
...
###Additonal Item##########
## Package KlaR help to create exploratory graph for LDA and QDA . It plots the features and partition the target class taking two combination of a variable at a time. The classifaction border are displayed in pink and the subsequent error rates are given as well. 
```{r}
library(klaR)
