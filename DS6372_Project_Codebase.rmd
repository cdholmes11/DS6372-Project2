...
packages <- c("aplore3", "tidyverse", "corrplot", "car", "caTools", "ROCR", "caret")
...
train_sub <-
  train %>%
  select(-c('sub_id', 'bmi', 'fracscore', 'frac_num', 'smoke', 'momfrac'))
...
# Final Model
log_model <- glm(fracture ~ priorfrac + age + height + bonemed_fu + bonetreat, data = train_sub, family = "binomial")
summary(log_model)
...
predict_test <- ifelse(predict_test > 0.5, "Yes", "No")
predict_val <- ifelse(predict_val > 0.5, "Yes", "No")
