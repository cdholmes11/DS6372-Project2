...
```
        ...
        ## heatmap data set for numerical variables only and selecting
```{r}
...
###########Loess plot for numerical variable, except the 
```{r}
df %>%
  ggplot(aes(x = age, y = frac_num)) +
  geom_point() +
...
mvn(dflda1,mvnTest ="mardia",univariateTest="AD",multivariatePlot ="qq",multivariateOutlierMethod="quan",showOutliers=T)
mvn(lq2,mvnTest ="mardia",univariateTest="AD",multivariatePlot ="qq",multivariateOutlierMethod="quan",showOutliers=T)
...
######EXTRA########
##3D plot for numeric variables
```{r}
plot3d( 
  x=df$height, y=df$age, z=df$weight, 
  col = df$color, 
  type = 's', 
  radius = 1,
    xlab="Height", ylab="Age", zlab="Weight")
...
##from ROC curve the threshold is 0.285~0.29 and AUC of 0.739. TPR(sensitivity),FPR(specificity) the value closer to (0,1) is determined as probabilty threshold . In this case the threshold is 0.285
...
```
              ...
              ###logloss metrics to compare the model######## removing fracscore and bmi give us lower log loss value
```{r}
...
## This figure shows how test data has been classified using LDA/QDA model. The mix of red and black colored shows incorrect classification prediction.
...
partimat(as.factor(fracture)~age+weight+height+fracscore+bmi,data =lq_train,method="lda",plot.matrix = TRUE,imageplot=TRUE)

partimat(as.factor(fracture)~age+weight+height+fracscore+bmi,data =lq_train,method="qda",plot.matrix = TRUE,imageplot=TRUE)
